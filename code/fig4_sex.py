...
gs=gridspec.GridSpec(1,4,width_ratios=[1,1,1.35,1],wspace=0.7,left=0.07,right=0.985,top=0.84,bottom=0.25)
...
axA.text(0.04,0.96,f'{len(j)} matched types\nρ = {J["matched_abs_spearman"]:.2f}',transform=axA.transAxes,va='top',fontsize=6)
axA.text(0.98,0.04,f'slope {k:.2f}',transform=axA.transAxes,ha='right',va='bottom',fontsize=6,color=C['female'])
...
axB.set_xticks(xx); axB.set_xticklabels(['axon\ncount','Σ diameter\n(r ~ d)','Σ area\n(r ~ d²)'],fontsize=5.5); axB.set_ylabel('descending share of channel'); axB.set_ylim(0,0.8)
axB.legend(loc='lower center',bbox_to_anchor=(0.5,1.0),ncol=1,fontsize=5.5,handlelength=1,borderaxespad=0.1)
...
axC.set_yscale('log'); plain_log(axC,'y'); axC.set_ylim(0.12,14); axC.set_xticks([0.5,1.5,2.5,3.5]); axC.set_xticklabels(['shared','specific','shared','specific'],fontsize=5.5); axC.set_xlim(0,4)
axC.set_ylabel('relative caliber of DNs'); axC.axhline(1,color=C['grey'],lw=0.5,ls='--')
axC.text(1.0,10.5,'male',ha='center',fontsize=6,color=C['male']); axC.text(3.0,10.5,'female',ha='center',fontsize=6,color=C['female'])
...
slots={'DNb05':4.6,'DNg37':3.4,'DNde002':2.5,'aSP22':1.85}
for t,r in lab.iterrows():
    axD.annotate(t,xy=(r.male,r.female),xytext=(0.19,slots.get(t,2.0)),fontsize=5,ha='left',va='center',arrowprops=dict(arrowstyle='-',lw=0.3,color=C['grey'],shrinkA=1,shrinkB=2,relpos=(1,0.5)))
axD.text(0.98,0.04,f'{len(d)} dimorphic types',transform=axD.transAxes,ha='right',va='bottom',fontsize=6)
