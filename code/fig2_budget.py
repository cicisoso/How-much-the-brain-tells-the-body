...
R='results'; ch=pd.read_parquet(f'{R}/channel_axons_male.parquet'); B=json.load(open(f'{R}/budget_male.json')); RB=json.load(open(f'{R}/robustness.json'))
fig=plt.figure(figsize=(180*MM,70*MM))
gs=gridspec.GridSpec(1,4,width_ratios=[1.3,1,1.25,1],wspace=0.6,left=0.075,right=0.985,top=0.8,bottom=0.24)
...
for sc in ['ascending_neuron','sensory_ascending','descending_neuron']:
    ...
    axA.axvline(d.median(),ymax=0.62,color=CLASS_COLOR[sc],lw=0.6,ls=':')
axA.set_xscale('log'); plain_log(axA,'x'); axA.set_xlabel('axon diameter at neck (µm)'); axA.set_ylabel('axons'); axA.set_ylim(0,275); axA.legend(loc='upper right',fontsize=8,handlelength=1.0,borderaxespad=0.1,labelspacing=0.25)
top=ch.sort_values('diam_um',ascending=False).drop_duplicates('type').head(2)
for _,r in top.iterrows(): axA.annotate(r.type.replace('DNp01','DNp01 (GF)'),xy=(r.diam_um,2),xytext=(r.diam_um,95 if r.type=='DNp01' else 50),fontsize=8,ha='center',rotation=90,va='bottom',arrowprops=dict(arrowstyle='-',lw=0.4,color=C['grey']))
...
n50=B['n_axons_for_half_area']; axB.plot([n50/len(a)],[0.5],'o',ms=3,color=C['AN']); axB.annotate(f'{n50} axons\n= 50% of area',xy=(n50/len(a),0.5),xytext=(0.36,0.2),fontsize=8,arrowprops=dict(arrowstyle='-',lw=0.4,color=C['grey']))
axB.set_xlabel('fraction of axons (thickest first)'); axB.set_ylabel('cumulative area fraction'); axB.set_xlim(0,1); axB.set_ylim(0,1); axB.text(0.62,0.06,f'Gini = {B["gini_area"]:.2f}',fontsize=8)
meas=['count','Σ d','Σ d²']
...
for i,f in enumerate(frac_dn): axC.text(i,f/2,f'{100*f:.0f}%',ha='center',va='center',color='white',fontsize=8); axC.text(i,f+(1-f)/2,f'{100*(1-f):.0f}%',ha='center',va='center',color='white',fontsize=8)
axC.set_xticks(xx); axC.set_xticklabels(meas); axC.set_ylabel('share of channel'); axC.set_ylim(0,1); axC.legend(loc='lower center',bbox_to_anchor=(0.5,1.0),ncol=1,fontsize=8,handlelength=1,columnspacing=0.8,borderaxespad=0.1,labelspacing=0.2)
...
lo,hi=RB['parity_beta_ci']; axD.plot([lo,hi],[1,1],color=C['AN'],lw=2.0,solid_capstyle='butt')
axD.set_xlabel('caliber–rate exponent β'); axD.set_ylabel('descending / ascending\ncapacity ratio'); axD.set_ylim(0.4,2.0)
axD.text(1.0,2.03,'empirical range',ha='center',va='bottom',fontsize=8,color=C['grey'],clip_on=False); axD.text(1.85,0.93,'parity',fontsize=8,color=C['grey'],va='top',ha='center')
