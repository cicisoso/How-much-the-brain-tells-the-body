...
fig=plt.figure(figsize=(180*MM,60*MM)); gs=gridspec.GridSpec(1,4,wspace=0.6,left=0.06,right=0.975,top=0.86,bottom=0.24)
...
axA.set_xlabel('diameter, anterior slab (µm)'); axA.set_ylabel('diameter, posterior slab (µm)'); axA.text(0.04,0.96,f'n = {len(gg)}\nρ = {stats.spearmanr(gg.A,gg.C)[0]:.2f}',transform=axA.transAxes,va='top',fontsize=8)
# B: tilt correction factor
ct=g[g.body.isin(ch.body)].cos_theta.dropna(); axB.hist(ct,bins=np.linspace(0.5,1,51),color=C['DN_light'],lw=0); axB.set_xlabel('cos θ (axon vs. slab normal)'); axB.set_ylabel('axon–slab pairs'); axB.text(0.04,0.96,f'median {ct.median():.3f}',transform=axB.transAxes,va='top',fontsize=8)
...
axC.set_xlabel('voxel area (µm²)'); axC.set_ylabel('mesh-section area (µm²)'); axC.text(0.04,0.96,f'n = {len(mv)}\nmedian ratio {np.median(mv.area_mesh_um2/mv.area_um2):.2f}',transform=axC.transAxes,va='top',fontsize=8)
...
axD.set_xlabel('skeleton diameter (µm)'); axD.set_ylabel('EM diameter (µm)'); axD.text(0.04,0.96,f'n = {len(m)}\nρ = {stats.spearmanr(m.r_skel_neck,m.diam_um)[0]:.2f}',transform=axD.transAxes,va='top',fontsize=8)
