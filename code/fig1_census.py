...
# ---- panel A: CNS silhouette from neck-crossing skeletons (x vs z, um)
...
# ---- panel B: cross-section image
z=np.load(f'{D}/neck_slabs/plane_B_z26976.npz'); pl=z['plane']; H=np.load(f'{R}/plane_B_hullmask.npz')['hull']
ids=np.unique(pl); cat=np.full(len(ids),5,dtype=np.uint8)  # 0 DN 1 AN 2 SA 3 SD 4 efferent 5 unannotated 6 extracellular
cls=dict(zip(cal[cal.neck_listed].body.astype(np.int64),cal[cal.neck_listed].superclass))
sup=dict(zip(ann.bodyId.astype(np.int64),ann.superclass.astype(str)))
...
rgb=pal[img]; rgb[~H]=np.array([1,1,1])*0.97  # outside bundle hull: faint
ys,xs=np.where(H); x0,x1,y0,y1=xs.min()-40,xs.max()+40,ys.min()-40,ys.max()+40
crop=rgb[y0:y1,x0:x1]; ext=[0,(x1-x0)*0.016,(y1-y0)*0.016,0]
# ---- panel C: L-R reproducibility
lst=cal[cal.neck_listed&cal.type.notna()&cal.somaSide.isin(['L','R'])]
t=lst.groupby(['type','somaSide']).diam_um.median().unstack().dropna()
rho=stats.spearmanr(t.L,t.R)[0]; r=stats.pearsonr(np.log10(t.L),np.log10(t.R))[0]
# ---- panel D: resolution / method validation (optional)
import os
mv=pd.read_parquet(f'{R}/mip_validation.parquet') if os.path.exists(f'{R}/mip_validation.parquet') else None
fig=plt.figure(figsize=(174*MM,92*MM))
gs=gridspec.GridSpec(2,3,width_ratios=[1.15,1.9,1.15],height_ratios=[1,1],wspace=0.5,hspace=0.55,left=0.075,right=0.99,top=0.88,bottom=0.1)
...
zn=54000*0.008; axA.axhline(zn,color=C['AN'],lw=0.8,ls='--'); axA.text(P[:,0].max()+8,zn,'neck\nplane',color=C['AN'],fontsize=6,va='center')
axA.invert_yaxis(); axA.set_aspect('equal'); axA.set_xlabel('x (µm)'); axA.set_ylabel('anterior–posterior (µm)')
axA.text(P[:,0].max()+8,P[:,2].min()+120,'brain',ha='left',fontsize=6.5,color=C['grey']); axA.text(P[:,0].max()+8,P[:,2].max()-200,'VNC',ha='left',fontsize=6.5,color=C['grey'])
...
axB.plot([ext[1]-14,ext[1]-4],[ext[2]-3,ext[2]-3],color='k',lw=1.5); axB.text(ext[1]-9,ext[2]-5,'10 µm',ha='center',va='bottom',fontsize=6)
gf=cal[(cal.type=='DNp01')]
# legend patches
...
axB.legend(handles=handles,loc='upper center',bbox_to_anchor=(0.5,-0.01),ncol=3,fontsize=6,handlelength=1,columnspacing=1.0)
axB.set_title('Neck connective cross-section (16 nm EM segmentation)',fontsize=7)
# C
...
axC.text(0.04,0.96,f'{len(t)} cell types\nρ = {rho:.2f}',transform=axC.transAxes,va='top',fontsize=6)
# D
if mv is not None:
    ...
    axD.text(0.04,0.96,f'{len(mv)} axons\nmedian ratio {mv.ratio.median():.2f}',transform=axD.transAxes,va='top',fontsize=6)
else:
    axD.axis('off')
panel_labels_tight(fig,[axA,axB,axC,axD],'ABCD')
base='figures/Figure1'; require_matplotlib_panel_alignment(fig,json_out=base+'.alignment.json',overlay_svg=base+'.alignment.svg',tolerance_pt=1.5,gutter_tolerance_pt=1.5,require_panel_labels=False,strict=True,axes=[axA,axB,axC,axD],panel_ids=['A','B','C','D'],column_groups=[['C','D']],
